...
public class DataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        try {
            ...
            log.error("=== DataSeeder FALLO: {} ===", e.getMessage(), e);
        }
    }

    private void seedAdmin() {
        log.info("Iniciando seed de admin...");
        try {
            long userCount = userRepository.count();
            log.info("Total de usuarios en BD: {}", userCount);

            var adminOpt = userRepository.findByEmail(adminEmail);
            if (adminOpt.isPresent()) {
                User admin = adminOpt.get();
                log.info("Admin encontrado con rol: {}", admin.getRole());
                if (admin.getRole() != Role.ADMIN) {
                    admin.setRole(Role.ADMIN);
                    userRepository.save(admin);
                    log.info("CORREGIDO: Rol de admin cambiado a ADMIN para: {}", adminEmail);
                } else {
                    log.info("Admin ya tiene rol ADMIN correctamente");
                }
            } else {
                log.info("Admin no encontrado, creando usuario admin...");
    ...
    private void seedChallenges() {
        log.info("Iniciando seed de challenges...");
        try {
            long count = challengeRepository.count();
            log.info("Total de challenges en BD: {}", count);
            ...
            if (count == 0) {
                ...
                log.info("Ya existen {} challenges, omitiendo seed", count);
            }
        } catch (Exception e) {
            log.error("ERROR en seedChallenges: {}", e.getMessage(), e);
        }
    }
}
